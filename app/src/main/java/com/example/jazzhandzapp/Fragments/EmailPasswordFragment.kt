...
import com.example.jazzhandzapp.databinding.SigninRegisterFragmentBinding
import kotlinx.android.synthetic.main.signin_register_fragment.*

class EmailPasswordFragment : SigninBaseFragment() {

    private lateinit var auth: FirebaseAuth

    private var _binding: SigninRegisterFragmentBinding? = null
    private val binding: SigninRegisterFragmentBinding
        get() = _binding!!

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        _binding = SigninRegisterFragmentBinding.inflate(inflater, container, false)
        return binding.root
    }
...
//                activity?.supportFragmentManager?.beginTransaction()?.remove(this@EmailPasswordFragment)?.commit()
//                childFragmentManager.beginTransaction().remove(this@EmailPasswordFragment).commit()
//                FragmentManager().beginTransaction().remove(EmailPasswordFragment()).commit()
...
//    }
    ...
    companion object {
        ...
        const val RC_MULTI_FACTOR = 9005
    }
}
